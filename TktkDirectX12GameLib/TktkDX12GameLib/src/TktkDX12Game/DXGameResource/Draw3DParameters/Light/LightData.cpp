...
namespace tktk
{
		...
		: m_lightCBuffer({ ambient, diffuse, speqular, position })
	{
		...
		m_createUploadBufferHandle = DX12GameManager::createUploadBuffer(UploadBufferInitParam::create(BufferType::constant, DX12GameManager::getSystemHandle(SystemCBufferType::Light), LightCBufferData()));
	}
