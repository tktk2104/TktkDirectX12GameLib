...
namespace tktk
{
	ComponentCollisionFuncRunner::ComponentCollisionFuncRunner(const ComponentBasePtr& componentPtr, ComponentCollisionFuncVTable* vtablePtr)
	...
	void ComponentCollisionFuncRunner::runCollideFunc(const ComponentCollisionFuncRunner& other)
	{
		...
		// 衝突判定関数を呼ぶ
		if (m_vtablePtr->isCollide(m_selfPtr, other.m_selfPtr))
		{
