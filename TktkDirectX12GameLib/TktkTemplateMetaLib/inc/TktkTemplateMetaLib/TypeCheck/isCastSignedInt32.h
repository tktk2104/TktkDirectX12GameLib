...
namespace
{
	// テンプレート引数の型が４バイト符号付き整数かどうか判別する（基底の型がintな列挙型も含む）
...
// 「CheckType」が４バイト符号付き整数型だったらコンパイル可能な型
template <class CheckType>
using IsSignedInt32 = std::enable_if_t<IsSignedInt32_v<CheckType>>*;

// 「CheckType」が４バイト符号付き整数型ではなかったらコンパイル可能な型
template <class CheckType>
using IsNotSignedInt32 = std::enable_if_t<!IsSignedInt32_v<CheckType>>*;
